...
const validateResume = (req , res , next) => {
    ...
    if(!req.body.position || req.body.position.length === 0) 
        errors.position = 'Поле Желаемая должноасть обязательное'
...
const isAuthorOfResume = async (req, res, next) =>{
    ...
    if(!resume)res.status(400).send({messages: 'Resume with that id is not exist'})
    else if(req.user.id === resume.userId) next()
    else res.status(403).send({messages: 'Access Forbiden'})
}
